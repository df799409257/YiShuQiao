...
public class JCVideoPlayer extends FrameLayout implements View.OnClickListener, SeekBar.OnSeekBarChangeListener,
        SurfaceHolder.Callback, View.OnTouchListener, JCMediaManager.JCMediaPlayerListener {
...
//        }
        ...
        if (!Utils.isWifi(getContext())) {
            Toast.makeText(getContext(), "当前网络为4G请手动点击播放", Toast.LENGTH_LONG).show();
        } else {
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        // 9-4 判断添加 (&&JCMediaManager.intance().mediaPlayer.isPlaying())
        if (fromUser && JCMediaManager.intance().mediaPlayer.isPlaying()) {
    ...
    public void onPrepared() {
        ...
        if (JCMediaManager.intance().mediaPlayer == null) {
            JCMediaManager.intance().mediaPlayer = new MediaPlayer();
        }
