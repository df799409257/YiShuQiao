 ...
 * 作者：admin
 *
 * 创建时间：2017-6-22 下午1:15:17
 *
 * 项目名称：YueGuiF
 *
 * 文件名称：BGARefreshViewHolder.java
 *
 * 类说明：继承该抽象类实现响应的抽象方法，做出各种下拉刷新效果
 * 参考BGANormalRefreshViewHolder、BGAStickinessRefreshViewHolder
 * 、BGAMoocStyleRefreshViewHolder、BGAMeiTuanRefreshViewHolder
 *
 */
public abstract class BGARefreshViewHolder {
     ...
     */
    public void setLoadMoreBackgroundColorRes(
            @ColorRes int loadMoreBackgroundColorRes) {
     ...
     */
    public void setLoadMoreBackgroundDrawableRes(
            @DrawableRes int loadMoreBackgroundDrawableRes) {
     ...
     */
    public void setRefreshViewBackgroundColorRes(
            @ColorRes int refreshViewBackgroundColorRes) {
     ...
     */
    public void setRefreshViewBackgroundDrawableRes(
            @DrawableRes int refreshViewBackgroundDrawableRes) {
    ...
    public View getLoadMoreFooterView() {
        ...
        if (mLoadMoreFooterView == null) {
            mLoadMoreFooterView = View.inflate(mContext,
                    R.layout.view_normal_refresh_footer, null);
            ...
            if (mLoadMoreBackgroundDrawableRes != -1) {
                mLoadMoreFooterView
                        .setBackgroundResource(mLoadMoreBackgroundDrawableRes);
            }
            mFooterStatusTv = (TextView) mLoadMoreFooterView
                    .findViewById(R.id.tv_normal_refresh_footer_status);
            mFooterChrysanthemumIv = (ImageView) mLoadMoreFooterView
                    .findViewById(R.id.iv_normal_refresh_footer_chrysanthemum);
            mFooterChrysanthemumAd = (AnimationDrawable) mFooterChrysanthemumIv
                    .getDrawable();
     ...
     *
     * @param scale
     *            下拉过程0 到 1，回弹过程1 到 0，没有加上弹簧距离移动时的比例
     * @param moveYDistance
     *            整个下拉刷新控件paddingTop变化的值，如果有弹簧距离，会大于整个下拉刷新控件的高度
     */
    ...
    public void setSpringDistanceScale(float springDistanceScale) {
        if (springDistanceScale < 0) {
            throw new RuntimeException(
                    "下拉刷新控件paddingTop的弹簧距离与下拉刷新控件高度的比值springDistanceScale不能小于0");
        }
