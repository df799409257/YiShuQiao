 ...
 * 作者：admin
 * <p>
 * 创建时间：2017-6-22 下午1:13:57
 * <p>
 * 项目名称：YueGuiF
 * <p>
 * 文件名称：BGARefreshLayout.java
 * <p>
 * 类说明：下拉刷新、上拉加载更多、可添加自定义（固定、可滑动）头部控件（例如慕课网app顶部的广告位）
 */
public class BGARefreshLayout extends LinearLayout {
    ...
    private void initWholeHeaderView() {
        ...
        mWholeHeaderView.setLayoutParams(new LayoutParams(
    ...
    public void onFinishInflate() {
        ...
        if (getChildCount() != 2) {
            throw new RuntimeException(BGARefreshLayout.class.getSimpleName()
                    + "必须有且只有一个子控件");
        }
    ...
    public void startChangeWholeHeaderViewPaddingTop(int distance) {
        ValueAnimator animator = ValueAnimator.ofInt(
                mWholeHeaderView.getPaddingTop(),
    ...
    private void initRefreshHeaderView() {
        ...
        if (mRefreshHeaderView != null) {
            mRefreshHeaderView.setLayoutParams(new LayoutParams(
                    LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT));

            mRefreshHeaderViewHeight = mRefreshViewHolder
                    .getRefreshHeaderViewHeight();
            mMinWholeHeaderViewPaddingTop = -mRefreshHeaderViewHeight;
            mMaxWholeHeaderViewPaddingTop = (int) (mRefreshHeaderViewHeight * mRefreshViewHolder
                    .getSpringDistanceScale());
    ...
    public void setCustomHeaderView(View customHeaderView, boolean scrollable) {
        ...
        if (mCustomHeaderView != null) {
            mCustomHeaderView.setLayoutParams(new LayoutParams(
                    LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT));
        ...
        if (!mIsInitedContentViewScrollListener && mLoadMoreFooterView != null) {
            ...
            if (mContentView instanceof AbsListView) {
                ...
                if (mAbsListView instanceof ListView) {
                    ...
                    frameLayout.addView(mLoadMoreFooterView, params);
                    ((ListView) mAbsListView).addFooterView(frameLayout);
                }
            } else if (mContentView instanceof RecyclerView) {
                mRecyclerView = (RecyclerView) mContentView;
            } else if (mContentView instanceof ScrollView) {
        ...
        if (mAbsListView != null) {
            try {
                ...
                Field field = AbsListView.class
                        .getDeclaredField("mOnScrollListener");
                field.setAccessible(true);
                // 开发者自定义的滚动监听器
                final AbsListView.OnScrollListener onScrollListener = (AbsListView.OnScrollListener) field
                        .get(mAbsListView);
                mAbsListView
                        .setOnScrollListener(new AbsListView.OnScrollListener() {
                            @Override
                            public void onScrollStateChanged(
                                    AbsListView absListView, int scrollState) {
    ...
    public boolean shouldHandleAbsListViewLoadingMore(AbsListView absListView) {
        if (mIsLoadingMore || mCurrentRefreshStatus == RefreshStatus.REFRESHING
                ...
                || absListView == null || absListView.getAdapter() == null
        ...
        if (absListView.getChildCount() > 0) {
            ...
            lastChildBottom = absListView.getChildAt(
                    absListView.getChildCount() - 1).getBottom();
        }
        return absListView.getLastVisiblePosition() == absListView.getAdapter()
                .getCount() - 1
    ...
    public boolean shouldHandleRecyclerViewLoadingMore(RecyclerView recyclerView) {
        ...
        if (manager instanceof LinearLayoutManager) {
            ...
            if (layoutManager.findLastCompletelyVisibleItemPosition() == recyclerView
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (!mIsLoadingMore
                    ...
                    if (Math.abs(event.getRawX() - mInterceptTouchDownX) < Math
                            .abs(interceptTouchMoveDistanceY)
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (mIsCustomHeaderViewScrollable
                && !isWholeHeaderViewCompleteInvisible()) {
    ...
    private boolean shouldInterceptToMoveCustomHeaderViewDown() {
        return isContentViewToTop() && mCustomHeaderView != null
        ...
        if (null != mRefreshHeaderView) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    if (mCustomHeaderView != null) {
                        mWholeHeaderViewDownPaddingTop = mWholeHeaderView
                                .getPaddingTop();
                    }
    ...
    private boolean handleActionMove(MotionEvent event) {
        ...
        if (mCustomHeaderView != null && mIsCustomHeaderViewScrollable
                && isCustomHeaderViewCompleteVisible() && mRefreshDownY == -1) {
            mRefreshDownY = (int) event.getY();
        }

        int refreshDiffY = (int) event.getY() - mRefreshDownY;
        refreshDiffY = (int) (refreshDiffY / mRefreshViewHolder
                .getPaddingTopScale());
        ...
        if (refreshDiffY > 0 && shouldHandleRefresh()
            ...
            if (paddingTop > 0
                ...
                float scale = 1 - paddingTop * 1.0f
                        / mMinWholeHeaderViewPaddingTop;
                /**
                 * 往下滑 paddingTop mMinWholeHeaderViewPaddingTop 到 0 scale 0 到 1
                 * 往上滑 paddingTop 0 到 mMinWholeHeaderViewPaddingTop scale 1 到 0
                 */
        ...
        if (mCustomHeaderView != null && mIsCustomHeaderViewScrollable) {
            ...
            if ((mPullDownRefreshEnable && !isWholeHeaderViewCompleteInvisible())
                ...
                int paddingTop = mWholeHeaderViewDownPaddingTop
                        + wholeHeaderDiffY;
                if (paddingTop < mMinWholeHeaderViewPaddingTop
                        - mCustomHeaderView.getMeasuredHeight()) {
                    paddingTop = mMinWholeHeaderViewPaddingTop
                            - mCustomHeaderView.getMeasuredHeight();
                }
                mWholeHeaderView.setPadding(0, paddingTop, 0, 0);
    ...
    private boolean handleActionUpOrCancel(MotionEvent event) {
        ...
        if (mCurrentRefreshStatus == RefreshStatus.PULL_DOWN
            ...
            if (mCustomHeaderView == null
                    ...
                    && mWholeHeaderView.getPaddingTop() < 0 && mWholeHeaderView
                    .getPaddingTop() > mMinWholeHeaderViewPaddingTop)) {
    ...
    private void hiddenRefreshHeaderView() {
        ValueAnimator animator = ValueAnimator
                .ofInt(mWholeHeaderView.getPaddingTop(),
                        mMinWholeHeaderViewPaddingTop);
        animator.setDuration(mRefreshViewHolder.getTopAnimDuration());
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
    ...
    public void beginLoadingMore() {
        if (!mIsLoadingMore && mLoadMoreFooterView != null
                && mLoadMoreFooterView.getVisibility() == View.VISIBLE
                && mDelegate != null
    ...
    public void closingLoadingMore() {
        if (mIsLoadingMore) {
            if (mIsShowLoadingMoreView) {
                // 避免WiFi环境下请求数据太快，加载更多控件一闪而过
                mHandler.postDelayed(mDelaycloseLoadingMoreViewTask, 300);
            } else {
                mIsLoadingMore = false;
            }
        }
    ...
    private Runnable mDelaycloseLoadingMoreViewTask = new Runnable() {
        @Override
        public void run() {
            mIsLoadingMore = false;
    ...
    public interface BGARefreshLayoutDelegate {
        ...
        /**
         * 开始加载更多。由于监听了ScrollView、RecyclerView、AbsListView滚动到底部的事件，
         * 所以这里采用返回boolean来处理是否加载更多。否则使用endLoadingMore方法会失效
         *
         * @param refreshLayout
         * @return 如果要开始加载更多则返回true，否则返回false。（返回false的场景：没有网络、一共只有x页数据并且已经加载了x页数据了
         * ）
         */
        boolean onBGARefreshLayoutBeginLoadingMore(
