...
public class RoundImageView extends ImageView {
	...
	protected void onDraw(Canvas canvas) {
		...
		// drawInsideBorder
		drawCircleBorder(canvas, cx, cy, radius + borderInsideThickness / 2,
				borderInsideColor, borderInsideThickness);
		// drawOutSideBorder
		drawCircleBorder(canvas, cx, cy, radius + borderInsideThickness
				+ borderOutsideThickness / 2, borderOutsideColor,
				borderOutsideThickness);
	...
	private Bitmap getCroppedRoundBitmap(Bitmap bmp, int radius) {
		...
		if (scaledSrcBmp != null && !scaledSrcBmp.isRecycled()) {
	 ...
	 * Draw circle border
	 */
	private void drawCircleBorder(Canvas canvas, float cx, float cy,
			float radius, int color, int thickness) {
		Paint paint = new Paint();
		/* Anti-aliasing */
		paint.setAntiAlias(true);
		paint.setFilterBitmap(true);
